...
function PositionEditor({ mode, selected, positions, saving, onClose, onSave }: { mode: Exclude<EditorMode, null>; selected?: OrganizationPosition; positions: OrganizationPosition[]; saving: boolean; onClose: () => void; onSave: (data: FormData) => Promise<void> }) {
  const eligibleParents = mode === "create" ? positions.filter((position) => position.level >= 4) : positions.filter((position) => position.id !== selected?.id);
  const defaultParent = mode === "edit" ? selected?.parentId : selected && selected.level >= 4 ? selected.id : eligibleParents[0]?.id;
