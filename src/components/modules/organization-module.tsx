...
export function OrganizationModule({ session }: { session: ActiveSession }) {
  ...
  async function savePosition(formData: FormData) {
    ...
    const name = normalizePositionName(String(formData.get("name") ?? ""));
    ...
    try {
      if (editorMode === "edit" && selected) {
        ...
        const response = await fetch("/api/organization/positions", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ mode: "manual", items: [{ clientId: crypto.randomUUID(), name, branch, parentId, processId, relationship, requestedLevel }] }) });
  ...
  return <>
    <section className="module-heading"><div><p className="module-kicker">Estructura organizacional</p><h2>Organización y puestos</h2><p>Organigrama vivo conectado con puestos, procesos y permisos, con puestos operativos desde el nivel 5.</p></div>{canManage ? <button className="button button-primary" type="button" onClick={() => setEditorMode("create")}><Plus size={16} /> Agregar puesto</button> : <span className="module-state module-state-vigente">Fuente vigente</span>}</section>
...
function PositionEditor({ mode, selected, positions, saving, onClose, onSave }: { mode: Exclude<EditorMode, null>; selected?: OrganizationPosition; positions: OrganizationPosition[]; saving: boolean; onClose: () => void; onSave: (data: FormData) => Promise<void> }) {
  const maximumLevel = Math.max(4, ...positions.map((position) => position.level));
  const availableTargetLevels = Array.from({ length: maximumLevel - 3 }, (_, index) => index + 5);
  const initialLevel = mode === "create" && selected && selected.level >= 4 ? selected.level + 1 : 5;
  const [requestedLevel, setRequestedLevel] = useState(initialLevel);
  const eligibleParents = mode === "create" ? positions.filter((position) => position.level === requestedLevel - 1) : positions.filter((position) => position.id !== selected?.id);
  const defaultParent = mode === "edit" ? selected?.parentId : selected?.level === requestedLevel - 1 ? selected.id : eligibleParents[0]?.id;
  const [parentId, setParentId] = useState(defaultParent ?? "");
  const parentPosition = positions.find((position) => position.id === parentId);
  const sourcePosition = mode === "create" ? parentPosition : selected;
  return <div className="organization-modal-backdrop" role="presentation"><form className="organization-modal" action={onSave}><header><div><small>{mode === "create" ? "Nuevo puesto subordinado" : "Editar puesto"}</small><h3>{mode === "create" ? "Agregar puesto al organigrama" : `${selected?.id} · ${selected?.name}`}</h3></div><button type="button" onClick={onClose} aria-label="Cerrar"><X size={18} /></button></header>{mode === "create" ? <div className="organization-parent-preview"><span>Nivel {requestedLevel}</span><p>{parentPosition ? <>Se agregará debajo de <strong>{parentPosition.name}</strong>, que pertenece al nivel {requestedLevel - 1}.</> : <>No existen puestos disponibles en el nivel {requestedLevel - 1}. Selecciona otro nivel.</>}</p></div> : null}<label>Nombre del puesto<input autoCapitalize="none" autoFocus name="name" defaultValue={mode === "edit" ? selected?.name : ""} onInput={(event) => { event.currentTarget.value = event.currentTarget.value.toLocaleLowerCase("es-MX"); }} placeholder="ej. supervisor de producción" required /></label><label>Área o rama<input name="branch" defaultValue={sourcePosition?.branch ?? ""} placeholder="Ej. Operaciones" required /></label>{mode === "create" ? <label>Nivel solicitado<select name="requestedLevel" value={requestedLevel} onChange={(event) => { const nextLevel = Number(event.target.value); const nextParents = positions.filter((position) => position.level === nextLevel - 1); setRequestedLevel(nextLevel); setParentId(nextParents[0]?.id ?? ""); }}>{availableTargetLevels.map((item) => <option key={item} value={item}>Nivel {item}</option>)}</select></label> : null}<label>Reporta a<select name="parentId" value={parentId} onChange={(event) => setParentId(event.target.value)} required={mode === "create"}>{mode === "edit" ? <option value="">Máxima autoridad</option> : null}{eligibleParents.sort((left, right) => left.level - right.level || left.name.localeCompare(right.name)).map((position) => <option key={position.id} value={position.id}>N{position.level} · {position.id} · {position.name}</option>)}</select></label>{mode === "create" ? <><label>Proceso vinculado<select name="processId" defaultValue={sourcePosition?.processLinks[0]?.processId ?? ""}><option value="">Sin proceso directo</option>{processCatalog.map((process) => <option key={process.id} value={process.id}>{process.id} · {process.name}</option>)}</select></label><label>Relación con el proceso<select name="relationship" defaultValue="participant"><option value="participant">Participante</option><option value="support">Soporte</option><option value="owner">Responsable</option><option value="approver">Aprobador</option></select></label></> : null}<p><ShieldCheck size={15} /> El nombre se guardará en minúsculas. El puesto superior debe pertenecer al nivel inmediato anterior.</p><footer><button className="button button-secondary" type="button" onClick={onClose}>Cancelar</button><button className="button button-primary" disabled={saving || (mode === "create" && !eligibleParents.length)} type="submit"><Check size={14} /> {saving ? "Guardando…" : mode === "create" ? "Crear puesto" : "Guardar cambios"}</button></footer></form></div>;
}
